...
2. Before broad search or live checks, invoke `context-discovery` and follow `skills/context-discovery/SKILL.md`, which owns the lookup order, its fallbacks, and the condition that makes discovery complete. Standard Codex and Claude children receive this activation from the existing generated child prompt; Cursor receives it from its always-on rule.
...
This is a completion gate, not only advice. Before a task closes, the shared completion decision looks at every repository its author admission granted and every Git worktree registered below the task directory, and refuses the close when one of them has a dirty working tree, holds commits on any local branch that no remote off this machine has, has no remote at all, has no remote except a directory on this machine, or has a remote that could not be asked. The refusal names the repository and either the uncommitted paths or each branch and its number of commits. Publication is established by asking each remote what it holds, not by reading local `refs/remotes/*`, which any command can write without sending anything; `TASK_AGENT_REMOTE_QUERY_TIMEOUT_SECONDS` bounds that call and defaults to 30 seconds. A remote whose `git ls-remote --get-url` answer is a directory on this machine is not asked at all and is named instead: pushing into the clone next door leaves every copy on one disk, which is the state seven of the twenty-five checkouts on this host are in. Deciding whether a remote's tip already contains a local branch is a walk over parents in this clone, and `refs/replace/*` and the graft file can rewrite that walk as cheaply as a remote-tracking ref, so the gate's Git commands run with `GIT_NO_REPLACE_OBJECTS=1` and an empty `GIT_GRAFT_FILE`. The claim the gate makes is therefore bounded to this: some remote that is not a directory on this machine reported these commits. Nothing walks the disk looking for such repositories: the task that made the change is the one place where it is still known which repositories those are.
