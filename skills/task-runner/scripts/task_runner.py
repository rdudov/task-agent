...
- Completion refuses a repository you were granted, or a Git worktree you created inside the task directory, whose working tree is dirty or whose commits no remote off this machine has — on every branch you leave behind, not only the one checked out, and measured by asking the remote rather than by your local `refs/remotes`. A remote that is a directory on this machine does not count: pushing there leaves every copy on this disk. Commit and push after verification.
