...
Installed entry points, including package files executed by absolute path,
resolve that task tree through `task_agent.task_runner.repo_root()`. The task
index must reuse that owner rather than deriving a different root from its
installed file depth; `TASKS_INDEX_ROOT` remains an index-test override, and
direct source-checkout scripts retain their previous `TASK_AGENT_ROOT` override
and checkout-relative fallback.
